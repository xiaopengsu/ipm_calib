...
namespace {
    const char *about = "-- ipm auto calib unused camera parameter; \n"
                        "-- run example: \n"
                        "./ipm_calib_video_un -v=../data/5.avi -dy=8 -dx=0 ";
    const char *keys =  "{v|../data/5.avi|input from video file or image}{dy|0|forward distance camera to field}{dx|0|left and right distance camera to field}";
}
...
int main(int argc, char *argv[]) {
    ...
    parser.about(about);
//    cout << "-----please read readme to input parameter-------" << " ";
    ...
    float dy,dx;
    if(parser.has("dy")) {
        dy = parser.get<float>("dy");
    }
    if(parser.has("dx")) {
        dx = parser.get<float>("dx");
    }
...
//                {
//                    fs<<"tx"<<Loacal[0];fs<<"ty"<<Loacal[1];fs<<"tz"<<Loacal[2];
//                    fs<<"pitch"<< Pose[0];fs<<"heading"<< Pose[1];fs<<"roll"<< Pose[3];
//                    fs<<"error_mean"<< error_mean;
//                    std::cout << "摄像机与地面坐标关系的标定结果已写入"<<tpfileName<<std::endl;
//                    fs.release();
//                }
////                cout<<"save t r"<<endl;
//            }
...
//        cout<<"calib_ipm_ok  " <<calib_ipm_ok<<endl;
        char key = (char) waitKey(waitTime);
